...
function M.loadVPK(func)
    ...
    if not __vpkPackage then
        ...
        if not __isloadingVPK then
            ...
            Dispatcher.dispatchEvent(EventType.Progress_Change_Wait, 0, Desc[1000040])
            VRFHelper.instance:LoadVPK(vpkPath, SettingsManager.getConfig("LANGUAGE"), function()
                __vpkPackage = true
                __isloadingVPK = false

                Dispatcher.dispatchEvent(EventType.Progress_Change_Wait, 100)
...
function M.load_model(path, func)
    if not __vpkPackage then
        return
    end
    return GltfHelper.LoadVModelRuntime(path, func)
end
